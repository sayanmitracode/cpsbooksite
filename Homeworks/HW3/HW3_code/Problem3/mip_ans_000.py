...
class Verifier:
    ...
    def create_inputs(self, x_test, perturbation):
        ...
        for i in range(len(x_test)):
            ...
            # YOUR CODE HERE
            pixelVar = self.gurobi_model.addVar(lb=x_test[i]-perturbation, ub=x_test[i]+perturbation)#, vtype=GRB.CONTINUOUS, name="pixle"+str(i))
            self.x_vars.append(pixelVar)
            # self.gurobi_model.addConstr(x_test[i]-perturbation <= pixelVar, "c"+str(i)+"l")
            # self.gurobi_model.addConstr(x_test[i]+perturbation >= pixelVar, "c"+str(i)+"u")
    ...
    def add_linear_layer(self, input_vars, weight, bias):
        ...
        print('wight:',len(weight))
        print('bias:',len(bias))
        print('input_vars:',len(input_vars))
        ...
        for i in range(output_size):
            # YOUR CODE HERE
            dot_product_result = self.gurobi_model.addVar(vtype=GRB.CONTINUOUS, name="tmp")
            for i in range(output_size):
                for j in range(len(input_vars)):
                    dot_product_result += weight[i][j] * input_vars[j]
                dot_product_result += bias[i]
                self.gurobi_model.addConstr(z[i]==dot_product_result)    
            # self.gurobi_model.addConstr(z[i]==gurobipy.LinExpr(weight[i],input_vars)+bias[i])      
            #pass
        self.gurobi_model.update()
    ...
    def add_relu_layer(self, z, lower_bounds, upper_bounds):
        ...
        for i in range(n_inputs):
            # YOUR CODE HERE
            if(lower_bounds[i]<0 and upper_bounds[i]>0):
                self.gurobi_model.addConstr(hatz_vars[i] <= z[i] - lower_bounds(1-p_vars[i]))
                self.gurobi_model.addConstr(hatz_vars[i] <= upper_bounds*p_vars[i])
                self.gurobi_model.addConstr(hatz_vars[i] >= z[i])
                self.gurobi_model.addConstr(hatz_vars[i] >= 0)
            elif (lower_bounds[i]>=0):
                self.gurobi_model.addConstr(hatz_vars[i] == z[i])
    ...
    def solve_objectives(self, objectives, direction='minimization'):
        ...
        for i in range(len(objectives)):
            ...
            assert self.gurobi_model.status==gurobipy.GRB.OPTIMAL #True  YOUR CODE HERE
