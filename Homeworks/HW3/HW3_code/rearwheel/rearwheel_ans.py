""" Kinematic Bicycle Model
    Written by: Chenxi Ji & Kristina Miller
...
def another_controller(z, u, k, path, vhcl_mdl, gamma = 0.025, S = 1001):
	...
	pos = np.array([x_r, y_r])
	goal_pos = None

	last_vec = path(s[-1]) - pos
	last_len = norm(last_vec)
	...
	if last_len <= gamma:
		...
		for i in np.flip(s): 
			dir_vec = path(i) - pos
			dir_len = norm(dir_vec)
			dir_angle = np.arctan2(dir_vec[1], dir_vec[0])
			if dir_len > L:
				continue
			else:
				goal_pos = path(i)
		...
		if np.any(goal_pos) == None:
			...
			alpha = np.arctan((y_ref - y_r)/(x_ref - x_r)) - theta
			w = 2*v_r*np.sin(alpha)/L

			delta = np.arctan(l*w/v_r)
			if np.absolute(delta) > delta_max:
...
def path(s):
    ...
    the input s is a parameter. Please don't change.
    '''
    if mode==1:
        x = 80*s
        ...
        x = 80 * s
        y = 4 * np.sin(4 * np.pi * s-np.pi/2) * np.exp(-2*s) + 4
    elif mode==4:
        x = 80 * s
        y = 8 * 4 * s * (1 - s) 
...
for mode in range(1, 5):
    ...
    path_control(z0, u0, t, 'another_controller', k1, path, vhcl_mdl)
